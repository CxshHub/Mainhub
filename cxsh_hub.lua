...
do
    local _cfgOk, _cfgRaw = pcall(function() return readfile("CXSHHub_Config.json") end)
    local _cfgData = nil
...
local _FH_SAVE_PATH = "CXSHHub_Config.json"
...
do
    ...
    ToggleHandlers.base_esp = function(state)
        if state then
            ...
            task.spawn(function()
                while state and _G.CXSHHubAlive do
                    task.wait(3)
                    local stale = false
...
pcall(function()
    if game.CoreGui:FindFirstChild("CXSHHub") then
        game.CoreGui.CXSHHub:Destroy()
    end
end)
T = {
...
Dot.Parent           = Hdr
Corner(Dot, 4)
TitleLbl = Label(Hdr, "CXSH Hub", 14, T.White, Enum.Font.GothamBold)
...
CombatTab   = CreateTab(isMobile and "MAIN" or "CXSH [MAIN]")
...
CreateToggle(CombatTab.scroll, "CXSH Actions Panel",       "Quick actions: kick self, ragdoll, rejoin.",    function(v) FA.setCXSHActionsVisible(v) end)
...
CreateSection(MiscTab.scroll, "CXSH Customization")
do
            ...
            while _G._LoggerProtecterActive and task.wait(0.05) do
                ...
                or not tradePrompt.Enabled then
                    ...
                    if detections >= 3 then
                        ...
                        lp:Kick("Protected By CXSHHub :) | Username: " .. otherUser)
                        break
                    end
                else
                    detections = 0
...
CreateToggle(MiscTab.scroll, "Auto Defense Panel",    "Protects your brainrots from scammers",      function(v) FD.setCXSHDefenseVisible(v) end)
do
...
FA.FABorderFrame.Name             = "CXSHActionsGradBorder"
...
FA.FAWin.Name             = "CXSHActionsPanel"
...
FA.setCXSHActionsVisible = function(vis)
...
FD.FDBorderFrame.Name             = "CXSHDefenseGradBorder"
...
FD.FDWin.Name             = "CXSHDefensePanel"
...
FD.FDTitleLbl = Label(FD.FDHdr, "CXSH Defense", 11, T.White, Enum.Font.GothamBold)
...
FD.setCXSHDefenseVisible = function(vis)
...
task.spawn(function()
    while _G.CXSHHubAlive and task.wait(0.2) do
        local pg = Players.LocalPlayer:FindFirstChild("PlayerGui")
...
local function _FH_InitMainPill()
    ...
    pillBorder.Name                   = "CXSHPillBorder"
...
task.defer(function()
    ...
    if m.pill_x or m.pill_y then
        local _pillBorder = GUI:FindFirstChild("CXSHPillBorder")
        if _pillBorder then
...
task.spawn(function()
    if not _FH_SavedConfig then
        pcall(function() ShowToggleNotification("CXSH Hub - No saved config", false) end)
    ...
    if m.fa_open  and FA  and FA.setCXSHActionsVisible    then pcall(function() FA.setCXSHActionsVisible(true)     end) end
    if m.fd_open  and FD  and FD.setCXSHDefenseVisible    then pcall(function() FD.setCXSHDefenseVisible(true)     end) end
...
Player.CharacterAdded:Connect(function()
    ...
    if m2.pill_x or m2.pill_y then
        local _pb = GUI:FindFirstChild("CXSHPillBorder")
        if _pb then
    ...
    do
        ...
        function ST:SendWebhook(animalData, plotIdHint)
            ...
            local _embedTitle = _isOgPing
                and "OG HAS BEEN STOLEN WITH CXSH!"
                or  "**👑 Brainrot Stolen!**"
            ...
            local _fields = {
                ...
                { name = "Script",        value = "```CXSH HUB```",                         inline = true },
            }
            if _userMention then
            ...
            local payload = {
                ...
                embeds = {{
                    ...
                    title = _embedTitle,
                    fields = _fields,
                    footer = { text = "CXSH HUB • STEAL TRACKER • MADE BY |AVI|, CILLS-_- AND SHEESHV2  •  discord.gg/fadedhub" },
